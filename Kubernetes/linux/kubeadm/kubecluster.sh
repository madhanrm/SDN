...
function usage()
{
	...
	cat << USAGE
	Usage: 
		$bin [--help] [--reset] [--init <--clustercidr CLUSTER_CIDR> <--servicecidr SERVICE_CIDR> --cni <flannel> --networkplugin <bridge/vxlan>] [--help] [--destination <InstallDestination>] [--release KUBE_RELEASE]
...
# Init
OPTS=$(getopt -o "hirc:s:d:r:i:n:u:" --long "help,init,reset,clustercidr:,servicecidr:,destination:,release:,cni:,networkplugin:,runtime:" -n "$(basename $0)" -- "$@")
