...
function DownloadKubeConfig($Master, $User="localadmin")
{
    $kc = GetKubeConfig
...
function InstallFlannelD()
{
    ...
    DownloadFile -Url  "https://github.com/$Global:GithubSDNRepository/raw/$Global:GithubSDNBranch/Kubernetes/flannel/$Global:NetworkMode/net-conf.json" -Destination $Global:BaseDir\net-conf.json
...
function InstallKubeProxy()
{
    ...
    $proxyArgs = GetProxyArguments -KubeConfig $KubeConfig  `
                    ...
                    -ProxyFeatureGates $KubeletFeatureGates
...
function CreateSCMService()
{
    ...
    $LogName = "Kubernetes-$ServiceName"
...
{
    return;
    # Pull Windows images for the corresponding build
    # Todo
    # For now assume the image exists
...
function GetKubeClusterInfo([ref]$ClusterCidr, [ref]$ServiceCidr)
{
...
{
    $out = (kubectl.exe cluster-info dump | findstr -i cluster-cidr)
    $m = ($out | Select-String -Pattern '--cluster-cidr=(.*)",' -AllMatches).Matches
    return ($m.Groups | select -Last 1).Value
}

function GetServiceCidr()
{
    $out = (kubectl.exe cluster-info dump | findstr -i service-cluster-ip-range)
    $m = ($out | Select-String -Pattern '--service-cluster-ip-range=(.*)",' -AllMatches).Matches
    return ($m[0].Groups | select -Last 1).Value
}
...
function InstallCNI($cni, $NetworkMode, $ManagementIp, $CniPath)
{
...
Export-ModuleMember GetKubeClusterInfo
