...
if ($Join.IsPresent)
{
    ...
    try {
        $ClusterCIDR = ""
        $ServiceCIDR = ""
        GetKubeClusterInfo ([ref]$ClusterCIDR) ([ref]$ServiceCIDR)
    ...
    InstallCNI -Cni $Cni -NetworkMode $Global:NetworkMode -ManagementIP $ManagementIp -CniPath $(GetCniPath)
    if ($Cni -eq "flannel")
    {
